...
def classic_shading_pdf(omega_i,omega_o,roughness,F0=0.04, diffuse_prob=0):
    ...
    return (1-diffuse_prob) * f_specular + diffuse_prob * f_diffuse
...
if __name__ == "__main__":
    ...
    theta_l = np.linspace(0, np.pi / 2, 500)
    phi_l = np.linspace(0, np.pi, 500)
    ...
    plt.title('2D conditioned test Distribution')
    plt.savefig('images/2d_gt_test_pdf.png')
