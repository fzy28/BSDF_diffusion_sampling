...
def neusample_init_hemi_spherical(WI_RES):
    wiX = torch.linspace(0,torch.pi/2, steps = WI_RES)
    wiY = torch.linspace(-torch.pi,torch.pi, steps = WI_RES)
    grid_z1, grid_z2 = torch.meshgrid(wiX, wiY)
    gridwi = torch.stack([grid_z1, grid_z2], dim = -1)
    light_dir = gridwi.reshape((-1, 2))
    return light_dir,torch.zeros_like(light_dir[...,0])
def neusample_init_all_spherical(WI_RES):
    wiX = torch.linspace(0,torch.pi, steps = WI_RES)
    wiY = torch.linspace(-torch.pi,torch.pi, steps = WI_RES)
    grid_z1, grid_z2 = torch.meshgrid(wiX, wiY)
    gridwi = torch.stack([grid_z1, grid_z2], dim = -1)
    light_dir = gridwi.reshape((-1, 2))
...
def stratified_sample_wo_hemisphere(num_samples):
    wo = stratified_sampling_2d(num_samples) 
    wo[:,0] = wo[:,0] * torch.pi / 2
    wo[:,1] = wo[:,1] * 2 * torch.pi - torch.pi
    return wo
def stratified_sample_wo_allsphere(num_samples):
    wo = stratified_sampling_2d(num_samples) 
    wo[:,0] = wo[:,0] * torch.pi 
    wo[:,1] = wo[:,1] * 2 * torch.pi - torch.pi
    return wo
def online_sampling_neusample(
    ...
    #camera_dir = torch.Tensor([0.0,0.5]).reshape(1,2).repeat(LOADING_SIZE,1)    
    ...
    with torch.no_grad():
        ...
        gt_pdf = rgb_pred.flatten().detach().cpu().numpy()
...
def online_sampling_neusample_hemisphere(brdf_func,light_dir, invalid_dirs,LOADING_SIZE,NUM_WI_SAMPLES):
    camera_dir = stratified_sample_wo_hemisphere(LOADING_SIZE)    
    # camera_dir = torch.Tensor([0.1, 1.2]).reshape(1,2).repeat(LOADING_SIZE,1)
    ...
    with torch.no_grad():
        ...
        # rgb_tmp = rgb_pred[0,:].reshape(256, 256)
        # wiX = torch.linspace(-np.pi,np.pi, steps = 256)
        # wiY = torch.linspace(0,np.pi/2, steps = 256)
        # plt.figure(figsize=(10,10))
        # plt.pcolormesh(wiX.numpy(), wiY.numpy(), rgb_tmp.cpu().numpy())
        
        gt_pdf = rgb_pred.flatten().detach().cpu().numpy()
        wi_samples = torch.Tensor(samplewi.samplewi(gt_pdf, LOADING_SIZE, NUM_WI_SAMPLES)).reshape(-1, 2)
        wi_samples[:,0] = (wi_samples[:,0] + 1) / 4 * np.pi
        wi_samples[:,1] = wi_samples[:,1] * np.pi
        
        # wi_samples_tmp = wi_samples.reshape(LOADING_SIZE, NUM_WI_SAMPLES, 2)
        # wi_samples_tmp = wi_samples_tmp[0, ...]
        # print(camera_dir[0])
        # plt.figure(figsize=(10,10))
        # plt.hist2d(wi_samples_tmp[:,0].flatten().cpu().numpy(), wi_samples_tmp[:,1].flatten().cpu().numpy(), bins=400,density=True)
        # plt.show()
...
def online_sampling_neusample_allshphere(brdf_func,light_dir, invalid_dirs,LOADING_SIZE,NUM_WI_SAMPLES):
    camera_dir = stratified_sample_wo_allsphere(LOADING_SIZE)    
    # camera_dir = torch.Tensor([0.1, 1.2]).reshape(1,2).repeat(LOADING_SIZE,1)
    ...
    with torch.no_grad():
        ...
        gt_pdf = rgb_pred.flatten().detach().cpu().numpy()
        wi_samples = torch.Tensor(samplewi.samplewi(gt_pdf, LOADING_SIZE, NUM_WI_SAMPLES)).reshape(-1, 2)
        wi_samples[:,0] = (wi_samples[:,0] + 1) / 2 * np.pi
        wi_samples[:,1] = wi_samples[:,1] * np.pi
...
def online_sampling_directsample(
    brdf_func,light_dir, invalid_dirs,LOADING_SIZE,NUM_WI_SAMPLES):
    camera_dir = stratified_sample_wo(LOADING_SIZE)    
    #camera_dir = torch.Tensor([0.5,0.5]).reshape(1,2).repeat(LOADING_SIZE,1)
...
def online_sampling(
    pdf_func,wo, linespace_o, batchsize=1, num_samples=2**24
):
    linespace_i = torch.ceil(torch.sqrt(torch.tensor(batchsize))).int()
    theta_l = torch.linspace(0, torch.pi / 2 , linespace_i+1).to(device) 
    phi_l = torch.linspace(-torch.pi, -torch.pi + 2 * torch.pi, linespace_i+1).to(device) 
    theta_i, phi_i = torch.meshgrid(theta_l[:-1], phi_l[:-1],indexing='ij')
    theta_i = theta_i.reshape(-1) + torch.rand(linespace_i * linespace_i).to(device) * (torch.pi / 2 / linespace_i)
    phi_i = phi_i.reshape(-1) + torch.rand(linespace_i * linespace_i).to(device) * (2 * torch.pi / linespace_i)
    theta_i = torch.clamp(theta_i, 0, np.pi / 2)
    phi_i = torch.clamp(phi_i, -np.pi, np.pi)
    indices = np.random.choice(linespace_i * linespace_i, batchsize, replace=False)
    theta_i_batch = theta_i[indices]
    phi_i_batch = phi_i[indices]
    # plt.figure(figsize=(6, 6))
    # plt.hist2d(theta_i_batch.cpu().numpy(), phi_i_batch.cpu().numpy(), bins=100,density=True)
    # plt.show()
    
    theta_o, phi_o = wo
    
    omega_o = torch.stack(
        [theta_o.repeat(batchsize),
    phi_o.repeat(batchsize)], axis=1
    )
    
    # theta_i_batch = torch.tensor([np.pi/12]).to(device).repeat(batchsize)
    # phi_i_batch = torch.tensor([np.pi/2]).to(device).repeat(batchsize)
    omega_i = torch.stack(
            [
                torch.repeat_interleave(theta_i_batch, theta_o.shape[0]),
                torch.repeat_interleave(phi_i_batch, phi_o.shape[0]),
            ],
            dim=1,
    )
    ...
    pdf = pdf_func(omega_i, omega_o)

    pdf = pdf.view(batchsize, (linespace_o + 1), (linespace_o + 1)).permute(1, 2, 0)
    # plt.figure(figsize=(6, 6))
    # plt.pcolormesh(theta_o.cpu().numpy().reshape((linespace_o+1),(linespace_o+1)), phi_o.cpu().numpy().reshape((linespace_o+1),(linespace_o+1)), pdf[:,:,0].cpu().numpy().reshape((linespace_o+1),(linespace_o+1)))
    # plt.show()
    pmf = (pdf[:-1, :, :] + pdf[1:, :, :]) / 2
    pmf = (pmf[:, :-1, :] + pmf[:, 1:, :]) / 2

    pmf = pmf.reshape(linespace_o * linespace_o, batchsize)
    pmf = pmf / torch.sum(pmf, axis=0, keepdims=True)
    cdf = torch.cumsum(pmf, axis=0)
    cdf = cdf.permute(1, 0)
    cdf_contiguous = cdf.contiguous()

    samples_indx = torch.stack([torch.searchsorted(cdf_contiguous[i,:], torch.rand(num_samples).to(device)) for i in range(cdf.shape[0])], dim=1)

    omega_o_samples = samples_withjitter(samples_indx, pdf, linespace_o)
    omega_o_samples = omega_o_samples.permute(2, 0, 1).reshape(-1, 2)
    # omega_o_samples = omega_o_samples.cpu().numpy()
    # plt.figure(figsize=(6, 6))

    # counts_target, xedges, yedges, c = plt.hist2d(omega_o_samples[:,0,0], omega_o_samples[:,1,0], bins=400,density=True)
    # plt.show()
    omega_o_samples_1 = omega_o_samples.clone()
    omega_o_samples_1[:,0] = -omega_o_samples[:,0]
    omega_o_samples_1[:,1] = omega_o_samples[:,1] + np.pi
    omega_o_samples_1[:,1] = torch.where(omega_o_samples_1[:,1] > np.pi,omega_o_samples_1[:,1] - 2 * np.pi,omega_o_samples_1[:,1])
    omega_o_samples = torch.cat([omega_o_samples,omega_o_samples_1],dim=0)
    omega_i = torch.stack(
        [
            torch.repeat_interleave(theta_i_batch, num_samples),
            torch.repeat_interleave(phi_i_batch, num_samples),
        ],
        dim=1,
    )
    omega_i = omega_i.repeat(2, 1)
...
if __name__ == "__main__":
    ...
    def pdf_func(omega_i, omega_o):
        return classic_shading_pdf_disk(omega_i, omega_o, 0.5)
