...
def neusample_init(WI_RES):
    wiX = torch.linspace(-2.0,2.0, steps = WI_RES)
    ...
    invalid_dirs_1 = torch.square(light_dir[...,0]+1) + torch.square(light_dir[...,1]) > 0.995
    invalid_dirs_2 = torch.square(light_dir[...,0]-1) + torch.square(light_dir[...,1]) > 0.995
    invalid_dirs = torch.logical_and(invalid_dirs_1,invalid_dirs_2)
...
def online_sampling_neusample(
    ...
    camera_dir = camera_dir[torch.randperm(camera_dir.size(0))]
    half_size = camera_dir.size(0) // 2
    reflect_dir = camera_dir[:half_size]
    reflect_dir[:, 0] = reflect_dir[:, 0] - 1
    trans_dir = camera_dir[half_size:]
    trans_dir[:, 0] = trans_dir[:, 0] + 1
    camera_dir = torch.cat([reflect_dir,trans_dir],dim=0)
    
    # camera_dir = torch.Tensor([-0.9, 0.1]).reshape(1,2).repeat(LOADING_SIZE,1)
    ...
    with torch.no_grad():
        ...
        # wiX = torch.linspace(-2.0,2.0, steps = 256)
        # wiY = torch.linspace(-1.0,1.0, steps = 256)
        # wiX, wiY = torch.meshgrid(wiX, wiY)
