...
def classic_shading_pdf(omega_i,omega_o,roughness,F0=0.04, diffuse_prob=0):
    ...
    return (1-diffuse_prob) * f_specular * cosine_term  + diffuse_prob * f_diffuse *cosine_term
...
if __name__ == "__main__":
    ...
    theta_l = np.linspace(-np.pi / 2, np.pi / 2, 150)
    ...
    from utils.analytical_brdf_torch import classic_shading_pdf as t
    import torch
    omega_i = torch.tensor(omega_i).float()
    omega_o = torch.tensor(omega_o).float()
    
    brdf = t(omega_i,omega_o,roughness)
    brdf = brdf.numpy()
